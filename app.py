...
# Define a new dark theme with white text for visibility
theme_css = """
    <style>
        body {
...
if selected_entry:
    ...
    st.subheader(f"📌 {selected_entry_data['title']}")
    ...
    if selected_entry_data.get("todo_list"):
        st.subheader("✅ Saved To-Do List")
        for task in selected_entry_data["todo_list"]:
            st.write(f"- {task}")

# Footer with your name
st.markdown("<hr>", unsafe_allow_html=True)
st.markdown("<center>💖 Developed by Ayesha Farooq 💖</center>", unsafe_allow_html=True)
