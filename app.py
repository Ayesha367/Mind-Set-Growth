...
#         <p>All rights reserved to Ayesha Farooq</p>
#     </div>
# """, unsafe_allow_html=True)                                                      
...
# Set Page Config
st.set_page_config(page_title="Digital Diary", page_icon="📖", layout="centered")
...
if theme == "Dark":
    ...
    st.markdown("""
        <style>
        body { background-color: #f0f0f5; color: black; }
        </style>
    """, unsafe_allow_html=True)
...
if password != "mysecurepassword":  # Change this to your preferred password
    st.warning("Incorrect password. Access Denied!")
    st.stop()

# Load or Initialize Diary
if "diary" not in st.session_state:
    st.session_state.diary = pd.DataFrame(columns=["Date", "Mood", "Tags", "Entry"])

st.title("📖 Digital Diary")
st.subheader("Write and save your thoughts")
...
if st.button("🎙️ Speak to Enter"):
    ...
    with sr.Microphone() as source:
        ...
        try:
            ...
            st.success("Voice-to-Text Successful!")
        except:
            st.error("Could not recognize voice. Try again.")

if st.button("💾 Save Entry"):
    new_entry = pd.DataFrame([[date, mood, tags, entry]], columns=["Date", "Mood", "Tags", "Entry"])
    st.session_state.diary = pd.concat([st.session_state.diary, new_entry], ignore_index=True)
    st.success("Diary Entry Saved!")

# Search & Filter Diary
st.sidebar.subheader("🔎 Search & Filter")
search_text = st.sidebar.text_input("Search by Keyword")
filter_mood = st.sidebar.selectbox("Filter by Mood", ["All"] + list(st.session_state.diary["Mood"].unique()))
filter_date = st.sidebar.date_input("Filter by Date", datetime.date.today())

filtered_diary = st.session_state.diary.copy()
if search_text:
    filtered_diary = filtered_diary[filtered_diary["Entry"].str.contains(search_text, case=False, na=False)]
if filter_mood != "All":
    filtered_diary = filtered_diary[filtered_diary["Mood"] == filter_mood]
if filter_date:
    filtered_diary = filtered_diary[filtered_diary["Date"] == filter_date]

st.subheader("📂 My Diary Entries")
st.dataframe(filtered_diary)
...
def save_as_pdf():
    ...
    pdf.cell(200, 10, "Digital Diary", ln=True, align="C")
    for _, row in st.session_state.diary.iterrows():
        pdf.multi_cell(0, 10, f"📅 Date: {row['Date']}\n😀 Mood: {row['Mood']}\n🏷️ Tags: {row['Tags']}\n📝 Entry: {row['Entry']}\n\n---\n")
    pdf.output("diary_entries.pdf")
    return "diary_entries.pdf"

if st.button("📥 Export as PDF"):
    pdf_file = save_as_pdf()
    st.success("Diary Exported as PDF!")
    st.download_button(label="📩 Download PDF", data=open(pdf_file, "rb"), file_name="My_Diary.pdf", mime="application/pdf")

# Footer
st.markdown("---")
st.markdown("<p style='text-align: center; font-size: 14px;'>Developed by <strong>Ayesha Farooq</strong></p>", unsafe_allow_html=True)
